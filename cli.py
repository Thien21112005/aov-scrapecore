"""
Command-line interface (CLI) for Arena of Valor (Liên Quân Mobile) image downloader.
"""

import os
import sys
import argparse
from scraper import AOVScraper, strip_accents
...
def print_banner():
    if HAS_RICH:
        banner = """[bold cyan]
  ╔═══════════════════════════════════════════════════════════════════════╗
  ║       LIÊN QUÂN MOBILE - TOOL CÀO ẢNH TƯỚNG & TRANG PHỤC HD          ║
...
"""
        ...
        print("=" * 72)
        print("       LIÊN QUÂN MOBILE - TOOL CÀO ẢNH TƯỚNG & TRANG PHỤC HD")
        print("             Website: lienquan.garena.vn/hoc-vien/tuong-skin/")
        print("=" * 72)
...
def display_heroes_table(heroes: list[dict]):
    """Display list of heroes in a nice table."""
    if HAS_RICH:
...
def run_download_with_progress(scraper: AOVScraper, heroes: list[dict], output_dir: str, image_types: list[str], threads: int):
    ...
    if HAS_RICH:
        with Progress(
            ...
            results = scraper.download_heroes_batch(
                heroes,
...
def interactive_menu(scraper: AOVScraper, default_output: str = "downloads", default_threads: int = 5):
    ...
    while True:
        ...
        print("  [5] Khởi chạy Giao diện đồ họa (GUI Desktop)")
        print("  [0] Thoát")
        print("-" * 50)
...
def main():
    parser = argparse.ArgumentParser(
        description="Tool cào và tải ảnh tướng, skin Liên Quân Mobile Full HD từ lienquan.garena.vn"
    )
    ...
    if args.all:
        print_banner()
        heroes = scraper.get_heroes()
        run_download_with_progress(scraper, heroes, args.output, image_types, args.threads)
        return
    ...
    if args.hero:
        ...
        if not matched:
            ...
            return
        run_download_with_progress(scraper, matched, args.output, image_types, args.threads)
        return
    ...
    interactive_menu(scraper, default_output=args.output, default_threads=args.threads)
